...
if __name__ == "__main__":
    ...
    for key in label_space.keys():
        ...
        if run_mode=="sample":
            train_nlp_cache = os.path.join(cache_dir, f"nlp/train_nlp_embeddings_{key}_{text_mode}_sample.pkl")
        elif run_mode=="full":
            train_nlp_cache = os.path.join(cache_dir, f"nlp/train_nlp_embeddings_{key}_{text_mode}.pkl")
        ...
        train_nlp = nlp_embedding(
            ...
            onto=onto,
            pooling='mean',  # 使用平均池化,
            name_flag=text_mode  # 使用GO名称作为文本
        )
        ...
        for epoch in range(epoch_num):
            ...
            if f > best_f1:
                ...
                ckpt_path = os.path.join(ckpt_dir, f"{ctime}main_text_{text_mode}_distill{key}_best.pt")
                torch.save({
                    'epoch': epoch,
    ...
    output_file = os.path.join(output_path, f"main_text_{text_mode}_distill{ctime}.txt")
    with open(output_file, 'w') as file_prec:
        ...
        file_prec.write(f"Model: main_text_{text_mode}_distill\n")
