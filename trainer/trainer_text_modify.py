...
from models.Text_modify import CustomModel,MultiLabelLoss

def create_model_and_optimizer(config, label_num,pos_weight):
    """创建模型、损失函数和优化器"""
    model = CustomModel(
        config['embed_dim'], 
        config['nlp_dim'],
    ...
    # criterion = FocalBCEWithLogitsLoss(pos_weight=pos_weight)
    # criterion = nn.BCEWithLogitsLoss()
    criterion = MultiLabelLoss(
        label_num=label_num,
        # class_freq=pos_weight,  # 传入类别频率
        use_focal=True,
        focal_gamma=2.0,
...
def train_one_epoch(model, train_dataloader, criterion, optimizer, epoch, epoch_num, key, 
    ...
    for batch_idx, batch_data in tqdm(enumerate(train_dataloader), 
        ...
        # 前向传播
        outputs = model(batch_embeddings,batch_nlp)
...
def validate(model, test_dataloader, ia_list, epoch, epoch_num, key):
    ...
    with torch.no_grad():
        for batch_idx, batch_data in tqdm(enumerate(test_dataloader),
            ...
            output = model(batch_embeddings,batch_nlp)
...
def train_model_for_ontology(config, key, train_dataloader, test_dataloader, label_num, ia_list, ctime, metrics_output_test,pos_weight):
    """为特定本体训练模型"""
    ...
    for epoch in range(config['epoch_num']):
        ...
        if metrics['f1'] > best_f1:
            ...
            ckpt_path = os.path.join(ckpt_dir, f"{ctime}text_modify_{key}_best.pt")
            torch.save({
                'epoch': epoch,
...
def save_results(config, metrics_output_test, seed, ctime):
    ...
    output_file = os.path.join(config['output_path'], f"text_modify_{ctime}.txt")
