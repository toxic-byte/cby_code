...
def train_one_epoch_efficient(model, train_dataloader, list_embedding, criterion, optimizer, 
                              epoch, epoch_num, key, sub_batch_size=128):
    """
    训练一个epoch (更高效的版本，使用子批次处理)
    适用于GO标签数量很大的情况
    """
    model.train()
    loss_mean = 0
    ...
    for batch_idx, batch_data in tqdm(enumerate(train_dataloader), 
        ...
        for i in range(batch_size):
            ...
            # 分批处理GO terms
            for go_start in range(0, num_go_terms, sub_batch_size):
                go_end = min(go_start + sub_batch_size, num_go_terms)
                go_batch_size = go_end - go_start
                
                # 为当前样本与当前批次的GO配对
                esm_repeated = batch_embeddings[i].unsqueeze(0).repeat(go_batch_size, 1)  # [go_batch_size, esm_dim]
                nlp_batch = list_embedding[go_start:go_end]  # [go_batch_size, nlp_dim]
                
                # 前向传播
                logits = model(esm_repeated, nlp_batch)  # [go_batch_size, 1]
                sample_outputs.append(logits.squeeze(-1))
            
            # 拼接该样本对所有GO的预测
            sample_output = torch.cat(sample_outputs, dim=0)  # [num_go_terms]
            outputs_list.append(sample_output)
        
        # 拼接所有样本的预测
        outputs = torch.stack(outputs_list, dim=0)  # [batch_size, num_go_terms]
...
def validate(model, test_dataloader, list_embedding, ia_list, epoch, epoch_num, key, 
    ...
    with torch.no_grad():
        for batch_idx, batch_data in tqdm(enumerate(test_dataloader),
            ...
            batch_embeddings = batch_data['embedding'].cuda()  # [batch_size, esm_dim]
            batch_labels = batch_data['labels']  # [batch_size, num_go_terms]
            batch_size = batch_embeddings.shape[0]
            ...
            for i in range(batch_size):
                ...
                for go_start in range(0, num_go_terms, sub_batch_size):
                    ...
                    esm_repeated = batch_embeddings[i].unsqueeze(0).repeat(go_batch_size, 1)
                    nlp_batch = list_embedding[go_start:go_end]
                    
                    logits = model(esm_repeated, nlp_batch)
                    sample_outputs.append(logits.squeeze(-1))
                
                sample_output = torch.cat(sample_outputs, dim=0)
                outputs_list.append(sample_output)
            
            # output = torch.stack(outputs_list, dim=0).cpu()  # [batch_size, num_go_terms]
            output = torch.stack(outputs_list, dim=0)  # [batch_size, num_go_terms]
