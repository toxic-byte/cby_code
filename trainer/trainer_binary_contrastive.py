...
from models.Binary_Contrastive import ContrastiveCustomModel,HybridLoss

def create_model_and_optimizer(config, pos_weight=None):
    """创建模型、损失函数和优化器"""
    model = ContrastiveCustomModel(
...
def train_one_epoch_efficient(model, train_dataloader, list_embedding, criterion, 
                               optimizer, epoch, epoch_num, key):
    """
    使用对比学习的训练函数
    """
    model.train()
    ...
    for batch_idx, batch_data in tqdm(enumerate(train_dataloader), 
        ...
        # 前向传播（返回特征用于对比学习）
        logits_flat, esm_proj, nlp_proj = model(esm_flat, nlp_flat, return_features=True)
        
        logits = logits_flat.reshape(batch_size, num_go_terms)
        labels_flat = batch_labels.reshape(-1)
        
        # 计算混合损失
        total_loss, cls_loss, con_loss = criterion(
            logits_flat, 
            labels_flat,
            esm_proj,
            nlp_proj
        ...
        con_loss_mean += con_loss.item()
        
        if (batch_idx + 1) % 50 == 0:
            print(f'{key}  Epoch [{epoch+1}/{epoch_num}], Step [{batch_idx+1}/{len(train_dataloader)}], '
                  f'Total Loss: {total_loss_mean/(batch_idx+1):.4f}, '
                  f'Cls Loss: {cls_loss_mean/(batch_idx+1):.4f}, '
                  f'Con Loss: {con_loss_mean/(batch_idx+1):.4f}')
    
    return total_loss_mean / len(train_dataloader)
...
def validate(model, test_dataloader, list_embedding, ia_list, epoch, epoch_num, key):
    ...
    with torch.no_grad():
        for batch_data in tqdm(test_dataloader, desc=f"Epoch {epoch+1}/{epoch_num} Validation"):
            ...
            logits_flat = model(esm_flat, nlp_flat, return_features=False)
...
def train_model_for_ontology(config, key, train_dataloader, test_dataloader, 
    ...
    for epoch in range(config['epoch_num']):
        ...
        if metrics['f1'] > best_f1:
            ...
            ckpt_dir = './ckpt/cafa5/binary_contrastive'
            os.makedirs(ckpt_dir, exist_ok=True)
            ckpt_path = os.path.join(ckpt_dir, f"{ctime}binary_contrastive_{key}_best.pt")
            torch.save({
                'epoch': epoch,
...
def save_results(config, metrics_output_test, seed, ctime):
    ...
    output_file = os.path.join(config['output_path'], f"binary_contrastive_{config.get('text_mode', 'default')}_{ctime}.txt")
