...
from utils.util import calACC, calF, evaluate_annotations,EarlyStopping
import os
from datetime import datetime
from paper.cby_code.models.Contrastive import CustomModel,GraphConvolution


def create_model_and_optimizer(config, label_num,adj_matrix,pos_weight=None):
    """创建模型、损失函数和优化器"""
    model = CustomModel(
    ...
    optimizer = torch.optim.Adam(
        list(model.parameters()) + list(model_GCN.parameters()), 
    ...
    for name, param in model_GCN.named_parameters():
        if param.requires_grad:
            print(f"model_GCN.{name}: {param.shape}")
            trainable_params += param.numel()
            
    print(f"Total trainable parameters: {trainable_params:,}")
    return model,model_GCN, criterion, optimizer, scheduler

def train_one_epoch_efficient(model, model_GCN,train_dataloader, list_embedding, criterion, optimizer, 
                              epoch, epoch_num, key):
    """优化版训练 - 使用批量矩阵运算"""
    model.train()
    model_GCN.train()
...
def validate(model, model_GCN,test_dataloader, list_embedding, ia_list, epoch, epoch_num, key):
    """优化版验证"""
    model.eval()
    model_GCN.eval()
    _labels = []
...
def train_model_for_ontology(config, key, train_dataloader, test_dataloader, label_num,
                            list_embedding, ia_list, ctime, 
                            metrics_output_test, adj_matrix,pos_weight=None):
    """为特定本体训练模型"""
    model,model_GCN, criterion, optimizer, scheduler = create_model_and_optimizer(
        config, label_num,adj_matrix,pos_weight)
    ...
    for epoch in range(config['epoch_num']):
        # 训练
        train_loss = train_fn(model, model_GCN,train_dataloader, list_embedding, criterion, 
                             optimizer, epoch, config['epoch_num'], key)
        scheduler.step()
        
        # 验证
        metrics = validate(model,model_GCN, test_dataloader, list_embedding, ia_list, epoch, 
        ...
        if metrics['f1'] > best_f1:
            ...
            ckpt_dir = './ckpt/cafa5/binary_gcn'
            os.makedirs(ckpt_dir, exist_ok=True)
            ckpt_path = os.path.join(ckpt_dir, f"{ctime}binary_gcn_{key}_best.pt")
            torch.save({
                'epoch': epoch,
...
def save_results(config, metrics_output_test, seed, ctime):
    ...
    output_file = os.path.join(config['output_path'], f"binary_gcn_modify_{config.get('text_mode', 'default')}_{ctime}.txt")
