...
from dataset import obo_graph,load_datasets,verify_alignment,process_labels_for_ontology,create_dataloaders,create_ontology_adjacency_matrix
...
def main():
    ...
    config = get_config(run_mode="full", text_mode="name")
    ...
    for key in label_space.keys():
        ...
        # 计算NLP embeddings
        train_nlp, test_nlp = compute_nlp_embeddings(
            config, nlp_model, key, train_id, test_id, training_labels, label_list, onto)
        ...
        model = train_model_for_ontology(
            config, key, train_dataloader, test_dataloader, label_num, ia_list, ctime, metrics_output_test,adj_matrix)
